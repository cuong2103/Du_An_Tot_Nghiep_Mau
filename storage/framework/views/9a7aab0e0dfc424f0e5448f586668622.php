...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.admin','data' => ['title' => 'Quản lý Lễ tân']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Quản lý Lễ tân']); ?>
    <div class="space-y-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <h2 class="text-2xl font-bold text-gray-800">Quản lý Lễ tân</h2>
            <a href="<?php echo e(route('admin.receptionists.create')); ?>" class="px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition flex items-center gap-2">
        ...
        <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
            <form action="<?php echo e(route('admin.receptionists.index')); ?>" method="GET" class="flex flex-col md:flex-row gap-4 items-end">
                ...
                <div class="flex gap-2 w-full md:w-auto">
                    ...
                    <?php if(request()->anyFilled(['search', 'department', 'status'])): ?>
                        <a href="<?php echo e(route('admin.receptionists.index')); ?>" class="px-4 py-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 flex items-center justify-center transition">
                            Đặt lại
                        </a>
                    <?php endif; ?>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-gray-600">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <?php $__empty_1 = true; $__currentLoopData = $receptionists; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $receptionist): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-4 py-3 text-center"><?php echo e($receptionists->firstItem() + $key); ?></td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm bg-orange-100 text-orange-600 shrink-0">
                                            <?php echo e($receptionist->avatar_initials ?? mb_substr($receptionist->full_name, 0, 1)); ?>

                                        </div>
                                        <div>
                                            <div class="font-bold text-gray-900"><?php echo e($receptionist->full_name); ?></div>
                                            <div class="text-xs text-gray-500" title="SĐT đăng nhập"><i class="fa-solid fa-phone mr-1"></i><?php echo e($receptionist->phone); ?></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3 font-mono text-gray-800">
                                    <?php echo e($receptionist->staffProfile?->employee_code ?? '—'); ?>

                                </td>
                                <td class="px-4 py-3">
                                    <?php echo e($receptionist->staffProfile?->position ?? '—'); ?>

                                </td>
                                <td class="px-4 py-3">
                                    <?php echo e($receptionist->staffProfile?->department ?? '—'); ?>

                                </td>
                                <td class="px-4 py-3 text-center">
                                    <?php echo e($receptionist->staffProfile?->internal_phone ?? '—'); ?>

                                </td>
                                <td class="px-4 py-3 text-center">
                                    <?php echo e($receptionist->staffProfile?->start_date ? \Carbon\Carbon::parse($receptionist->staffProfile->start_date)->format('d/m/Y') : '—'); ?>

                                </td>
                                <td class="px-4 py-3 text-center">
                                    <?php if($receptionist->is_active): ?>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="<?php echo e(route('admin.receptionists.show', $receptionist->id)); ?>" class="w-8 h-8 rounded bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-blue-100 transition" title="Xem chi tiết">
                                            <i class="fa-solid fa-eye"></i>
                                        </a>
                                        <a href="<?php echo e(route('admin.receptionists.edit', $receptionist->id)); ?>" class="w-8 h-8 rounded bg-yellow-50 text-yellow-600 flex items-center justify-center hover:bg-yellow-100 transition" title="Chỉnh sửa">
                                            <i class="fa-solid fa-pen"></i>
                                        </a>
                                        <form action="<?php echo e(route('admin.receptionists.toggle-active', $receptionist->id)); ?>" method="POST" onsubmit="return confirm('Bạn có chắc muốn thay đổi trạng thái tài khoản lễ tân này?');" class="inline">
                                            <?php echo csrf_field(); ?>
...
<?php endif; ?>
<?php /**PATH F:\Du_An_Tot_Nghiep_Mau\resources\views/admin/receptionists/index.blade.php ENDPATH**/ ?>