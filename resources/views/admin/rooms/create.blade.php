<x-layouts.admin title="Thêm Phòng khám mới">
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold text-gray-900 mt-2">Thêm Phòng khám mới</h2>
    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="room_type" required class="block w-full py-2 px-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-sm outline-none bg-white">
                            ...
                            <option value="examination" {{ old('room_type') == 'examination' ? 'selected' : '' }}>Phòng khám</option>
                            <option value="diagnostic" {{ old('room_type') == 'diagnostic' ? 'selected' : '' }}>Cận lâm sàng</option>
                            <option value="surgery" {{ old('room_type') == 'surgery' ? 'selected' : '' }}>Phẫu thuật</option>
                            <option value="other" {{ old('room_type') == 'other' ? 'selected' : '' }}>Khác</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select name="building" class="block w-full py-2 px-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-sm outline-none bg-white">
                            ...
                            <option value="Nhà K1" {{ old('building') == 'Nhà K1' ? 'selected' : '' }}>Nhà K1</option>
                            <option value="Nhà K2" {{ old('building') == 'Nhà K2' ? 'selected' : '' }}>Nhà K2</option>
                            <option value="Nhà K3" {{ old('building') == 'Nhà K3' ? 'selected' : '' }}>Nhà K3</option>
                            <option value="Khác" {{ old('building') == 'Khác' ? 'selected' : '' }}>Khác</option>
                        </select>
                        ...
                        @foreach($specialties as $sp)
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="specialty_ids[]" value="{{ $sp->id }}" 
                                    {{ in_array($sp->id, old('specialty_ids', [])) ? 'checked' : '' }}
