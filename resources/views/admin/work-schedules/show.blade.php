<x-layouts.admin title="Chi tiết ca làm việc">
    <div class="mb-6">
        <a href="{{ route('admin.work-schedules.index') }}" class="text-sm font-medium text-gray-500 hover:text-blue-600 transition-colors mb-2 inline-block">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-8">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-gray-500 text-sm mt-1">
                        ...
                        <i class="fa-solid fa-envelope mr-1"></i> {{ $schedule->doctor->user->email ?? 'Không có email' }}
                    </p>
                </div>
            </div>
            ...
            <div class="flex flex-col md:items-end gap-2">
                ...
                @endif
                <form action="{{ route('admin.work-schedules.toggle-active', $schedule->id) }}" method="POST" class="mt-1">
                    @csrf @method('PATCH')
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-8 pt-6 border-t border-gray-100">
            <div>
                ...
                <div class="font-semibold text-gray-900">
                    ...
                    <span class="text-lg">{{ substr($schedule->start_time, 0, 5) }} - {{ substr($schedule->end_time, 0, 5) }}</span>
                </div>
            </div>
            <div>
            ...
            <div>
                <span class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Thời lượng/Slot</span>
    ...
    <h3 class="text-lg font-bold text-gray-900 mb-4 mt-8">
        ...
        Lịch làm việc định kỳ trong tuần
    </h3>
        ...
        <div class="min-w-[800px] grid grid-cols-7 gap-3">
            ...
            @foreach($days as $dayNum => $dayName)
                <div class="flex flex-col">
                    <div class="bg-gray-50 border border-gray-200 text-center py-2 rounded-t-lg text-sm font-bold text-gray-700 {{ $schedule->day_of_week == $dayNum ? 'bg-blue-100 text-blue-800 border-blue-200' : '' }}">
                        {{ $dayName }}
                    </div>
                    <div class="border border-t-0 border-gray-200 rounded-b-lg p-2 flex-1 flex flex-col gap-2 min-h-[120px] {{ $schedule->day_of_week == $dayNum ? 'bg-blue-50/50 border-blue-200' : 'bg-gray-50/30' }}">
                        @if(isset($weeklySchedules[$dayNum]))
                            @foreach($weeklySchedules[$dayNum] as $ws)
                                <a href="{{ route('admin.work-schedules.show', $ws->id) }}" class="block bg-white border p-2.5 rounded shadow-sm hover:shadow-md transition-shadow cursor-pointer {{ $ws->id == $schedule->id ? 'border-blue-400 ring-1 ring-blue-400' : 'border-gray-200 hover:border-blue-300' }}">
                                    <div class="font-bold text-gray-900 text-xs flex items-center gap-1">
                                        <i class="fa-regular fa-clock text-gray-400"></i> {{ substr($ws->start_time, 0, 5) }} - {{ substr($ws->end_time, 0, 5) }}
                                    </div>
                                    <div class="text-gray-500 mt-1.5 text-[11px] truncate flex items-center gap-1" title="{{ $ws->room->name }}">
                                        <i class="fa-solid fa-door-open text-gray-400"></i> P.{{ $ws->room->room_number ?? $ws->room->name }}
                                    </div>
                                </a>
                            @endforeach
                        @else
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ngày khám</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <p class="text-sm text-gray-700 truncate max-w-xs" title="{{ $apt->reason }}">{{ $apt->reason }}</p>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="{{ route('admin.appointments.show', $apt->id) }}" class="text-blue-600 hover:text-blue-900 transition-colors">Xem phiếu hẹn <i class="fa-solid fa-arrow-right ml-1"></i></a>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm font-medium text-gray-900">Ca trực này hiện chưa có bệnh nhân nào đặt lịch sắp tới.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
