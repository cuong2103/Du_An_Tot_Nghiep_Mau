<x-layouts.admin title="Chỉnh sửa bài viết">
    <div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div class="text-right text-xs text-gray-500 font-medium bg-white px-4 py-2 rounded-lg border border-gray-100 shadow-sm">
            <div><i class="fa-regular fa-clock mr-1"></i> Ngày tạo: {{ $post->created_at->format('d/m/Y H:i') }}</div>
            <div class="mt-1"><i class="fa-regular fa-eye mr-1"></i> Lượt xem: <span class="text-blue-600 font-bold">{{ number_format($post->view_count) }}</span></div>
        </div>
    </div>
    ...
    <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" x-data="{
        title: '{{ old('title', addslashes($post->title)) }}',
        slug: '{{ old('slug', $post->slug) }}',
        thumbnailUrl: '{{ old('thumbnail_url', $post->thumbnail_url) }}',
        generateSlug() {
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">
                                URL hiện tại: <span class="text-blue-600 font-mono">{{ url('/posts') }}/<span x-text="generateSlug() || '...' "></span></span>
                            </p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Tóm tắt ngắn gọn</label>
                            <textarea name="summary" rows="3" placeholder="Đoạn mô tả ngắn hiển thị ở trang chủ và SEO..." class="block w-full py-2 px-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-sm outline-none">{{ old('summary', $post->summary) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="content" rows="15" required class="block w-full py-3 px-4 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-sm outline-none font-mono">{{ old('content', $post->content) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="w-full lg:w-1/3 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="text-xs text-gray-500 italic mt-[-8px]">
                            @if($post->is_published && $post->published_at)
                                Đã xuất bản lúc: {{ \Carbon\Carbon::parse($post->published_at)->format('H:i d/m/Y') }}
                            @else
                                Đang là bản nháp.
                            @endif
                        </div>
                        ...
                        <div>
                            ...
                            <select name="post_type" required class="block w-full py-2 px-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-sm outline-none bg-white">
                                ...
                                <option value="service" {{ old('post_type', $post->post_type) === 'service' ? 'selected' : '' }}>Dịch vụ khám chữa bệnh</option>
                                <option value="guide" {{ old('post_type', $post->post_type) === 'guide' ? 'selected' : '' }}>Hướng dẫn quy trình</option>
